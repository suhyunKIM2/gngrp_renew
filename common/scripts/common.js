...
//카테고리ID Formatting CXXX / bbs_manager_view.jsp, bbs_admin_form.jsp
    ...
    var beforePrint = function() {
        ...
        // 공문인쇄 추가조건
        var gongmoonHeader = $("#gongmoonHeader");
...
function closeDoc2(win) {
	...
	if (isOpener && isWindow) {
		...
		if (className.toLowerCase().indexOf("dhtmlwindow") > -1) {
			try { parent.modalwindow.hide(); } catch(e) {
...
function waitMsg(msg) {
	...
	var tag;
	tag  = '<h1 style="position: relative; top: -2px; font-weights:normal;">';
//	tag += '<img style="position: relative; top: 5px;" src="/common/jquery/css/validate/loading.gif" />';
//	tag += ' Processing  .....  Wait';
	tag += '<div style="background-color:#fff; position:relative; dtop:-49px; margin:5px 12px; ';
	tag += 'height:56px; font-size:17px; padding:7px 0px 0px; z-index:1000;">';
//	tag += '<div><img style="position: relative; top: 12px;" src="/common/images/loading_img.gif" /></div>';
	tag += '<div style="font-size:17px; background: #fff; padding: 8px;">';
	tag += '<img style="position: relative; top: 5px;" src="/common/jquery/css/validate/loading.gif" /> '+message+' </div>';
	tag += '</div>';
...
//Get Editor Value
function geteditordata___() {
	var editorValue = '';

	try { setEditorForm(); } catch(e) {}; // daum editor form tag create
	
	var tagfreeEditor = document.getElementById("twe"); // tagfree
	var daumEditor = document.getElementById("txtContent"); // daum
	
	if (tagfreeEditor) {
		editorValue = tagfreeEditor.MimeValue();
	} else if (daumEditor) {
		editorValue = daumEditor.value || '';
	} else {
		try { console.log('Error - Editor Object Not Found'); } catch(e) {}
		editorValue = 'Error - Editor Object Not Found';
	}
	return editorValue;
}
...
function geteditordata() {
	...
	if (tagfreeEditor) {
		...
		try { setEditorForm(); } catch(e) {}; // daum editor form tag create
		var daumEditor = document.getElementById("txtContent");
		editorValue = daumEditor.value || '';
	} //else {
		//alert( 'geteditordata() Error - Editor Object Not Found : Not Save' );
		//editorValue = 'Error - Editor Object Not Found';
	//}
	return editorValue;
}

//Editor 공통함수 : 본문 Html값 에디터에 설정하는 부분 - 기존함수 점검
function SetEditorData(objEditor, contentId) {
	var contents = "";
	if (!contentId) {
		contents = document.getElementById('bbs.content').value;
	} else {
